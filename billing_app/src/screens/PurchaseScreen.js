...
import {
  ...
  FlatList, Alert, Modal, ScrollView, Animated, KeyboardAvoidingView, Platform
} from 'react-native';
...
export default function PurchaseScreen() {
  const { state, dispatch, completePurchase, addProduct, updateProduct, addVendor, t } = useApp();
  ...
  const [vendorForm, setVendorForm] = useState({ name: '', phone: '', pan: '', gst: '' });
  ...
  const totals = useMemo(() => {
    ...
    return { subtotal, gst, totalWeight, grandTotal: Math.max(0, subtotal + gst - discount), discount };
  ...
  const generatePurchase = async () => {
    ...
    const sale = {
      ...
      grandTotal: totals.grandTotal, discount: totals.discount, totalWeight: totals.totalWeight,
      billDiscount, paymentMode, cashPaid: totals.grandTotal,
      vendorId: vendorId || null,
      warehouseId
    };
    try {
      await completePurchase(sale);
      setReceiptSale(sale);
    ...
    return (
      ...
      >
        ...
        <View style={{ alignItems: 'flex-end' }}>
          ...
          {inCart && <View style={styles.inCartBadge}><Text style={styles.inCartText}>{t('In Cart')}</Text></View>}
        </View>
      </TouchableOpacity>
    );
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
      ...
      {/* ── Product browse panel (only show if searching) ── */}
      <View style={{ paddingHorizontal: 15, zIndex: 10 }}>
        <TextInput
          style={styles.productSearchInput}
          placeholder={t('Search products manually...')}
          placeholderTextColor="#94A3B8"
          value={productSearch}
          onChangeText={setProductSearch}
        />
        {productSearch.length > 0 && (
          <View style={{ backgroundColor: '#fff', borderRadius: 8, elevation: 4, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 4, shadowOffset: { width: 0, height: 2 }, maxHeight: 250, position: 'absolute', top: 55, left: 15, right: 15, zIndex: 20 }}>
            <FlatList
              ...
              ListEmptyComponent={
                <Text style={{ textAlign: 'center', color: '#94A3B8', padding: 20 }}>{t('No products found')}</Text>
              }
            />
          </View>
        )}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyCart}>
              ...
              <Text style={styles.emptyText}>{t('Scan or search to add items to the cart')}</Text>
            </View>
          }
        />
      </View>
      ...
      <View style={styles.totalsCard}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, backgroundColor: '#f1f5f9', padding: 8, borderRadius: 6 }}>
          <Text style={{ fontSize: 13, color: '#475569', fontWeight: 'bold' }}>
            Receive to: {warehouseId ? displayWarehouses.find(w => w.id === warehouseId)?.name : 'Main Store'}
          </Text>
        ...
        {paymentMode === 'Debt' && (
           <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, backgroundColor: '#f1f5f9', padding: 8, borderRadius: 6 }}>
             <Text style={{ fontSize: 13, color: '#475569', fontWeight: 'bold' }}>
               Customer: {vendorId ? state.vendors.find(c => c.id === vendorId)?.name : 'None Selected'}
             </Text>
             <TouchableOpacity style={{ backgroundColor: '#2563eb', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 4 }} onPress={() => setShowVendorModal(true)}>
               <Text style={{ color: '#fff', fontSize: 12, fontWeight: 'bold' }}>{t('Change')}</Text>
             </TouchableOpacity>
           </View>
        )}
        ...
        {(paymentMode === 'Cheque' || paymentMode === 'RTGS/NEFT' || paymentMode === 'Bank') && (
           <TextInput 
             style={{ backgroundColor: '#fff', borderWidth: 1, borderColor: '#cbd5e1', padding: 10, borderRadius: 6, marginBottom: 10, fontSize: 13, color: '#0F172A' }}
             placeholder={paymentMode === 'Cheque' ? "Enter your Cheque No, Bank Name, Date" : paymentMode === 'Bank' ? "Enter your Bank Name, Transaction ID" : "Enter your UTR Number, Date"}
             placeholderTextColor="#94A3B8"
             value={bankInfo}
          ...
          <View style={styles.paymentModes}>
            {['Cash', 'UPI', 'Bank', 'Debt', 'Cheque', 'RTGS/NEFT'].map(m => (
              <TouchableOpacity
                ...
                onPress={() => {
                   setPaymentMode(m);
                   if (m === 'Debt' && !vendorId) setShowVendorModal(true);
                }}
              >
      ...
      <Modal visible={showVendorModal} animationType="slide" transparent={true} onRequestClose={() => setShowVendorModal(false)}>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: '#fff', borderTopLeftRadius: 16, borderTopRightRadius: 16, padding: 20, maxHeight: '80%' }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 }}>
              ...
              <TouchableOpacity onPress={() => setShowVendorModal(false)}><X size={24} color="#64748B" /></TouchableOpacity>
            </View>
            
            <View style={{ gap: 10, marginBottom: 15 }}>
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <TextInput style={{ flex: 1, borderWidth: 1, borderColor: '#cbd5e1', padding: 10, borderRadius: 6, color: '#0F172A' }} placeholder={t('New Vendor Name')} placeholderTextColor="#94A3B8" value={vendorForm.name} onChangeText={t => setVendorForm(p => ({...p, name: t}))} />
                <TextInput style={{ flex: 1, borderWidth: 1, borderColor: '#cbd5e1', padding: 10, borderRadius: 6, color: '#0F172A' }} placeholder={t('Phone (opt)')} placeholderTextColor="#94A3B8" value={vendorForm.phone} onChangeText={t => setVendorForm(p => ({...p, phone: t}))} keyboardType="numeric" />
              </View>
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <TextInput style={{ flex: 1, borderWidth: 1, borderColor: '#cbd5e1', padding: 10, borderRadius: 6, color: '#0F172A' }} placeholder={t('PAN Number')} placeholderTextColor="#94A3B8" value={vendorForm.pan} onChangeText={t => setVendorForm(p => ({...p, pan: t}))} autoCapitalize="characters" />
                <TextInput style={{ flex: 1, borderWidth: 1, borderColor: '#cbd5e1', padding: 10, borderRadius: 6, color: '#0F172A' }} placeholder={t('GST Number')} placeholderTextColor="#94A3B8" value={vendorForm.gst} onChangeText={t => setVendorForm(p => ({...p, gst: t}))} autoCapitalize="characters" />
                <TouchableOpacity style={{ backgroundColor: '#10b981', paddingHorizontal: 15, justifyContent: 'center', borderRadius: 6 }} onPress={async () => {
                  if (!vendorForm.name) return Alert.alert('Error', 'Name required');
                  const id = Date.now().toString();
                  await addVendor({ id, name: vendorForm.name, phone: vendorForm.phone, pan: vendorForm.pan, gst: vendorForm.gst, balance: 0 });
                  setVendorId(id);
                  setVendorForm({ name: '', phone: '', pan: '', gst: '' });
            ...
            <FlatList 
              ...
              renderItem={({item}) => (
                 <TouchableOpacity style={{ padding: 15, borderBottomWidth: 1, borderColor: '#f1f5f9' }} onPress={() => { setVendorId(item.id); setShowVendorModal(false); }}>
          ...
          <ScrollView contentContainerStyle={styles.receiptScroll}>
            <View style={styles.receiptCard}>
              <View style={styles.receiptTop}>
                ...
                <Text style={styles.storeName}>{t('Cosmo Store')}</Text>
                <Text style={styles.receiptSubtitle}>{t('Retail Invoice')}</Text>
              </View>

              <View style={styles.receiptDetails}>
                <View style={styles.receiptRow}>
                  <Text style={styles.receiptLabel}>{t('Bill No:')}</Text>
                ...
                <View style={styles.receiptRow}>
                  <Text style={styles.receiptLabel}>{t('Payment:')}</Text>
              ...
              <View style={styles.receiptTotals}>
                <View style={styles.receiptRow}><Text style={styles.receiptLabel}>{t('Subtotal')}</Text><Text style={styles.receiptValue}>₹{receiptSale?.subtotal.toFixed(2)}</Text></View>
                <View style={styles.receiptRow}><Text style={styles.receiptLabel}>{t('GST')}</Text><Text style={styles.receiptValue}>₹{receiptSale?.gst.toFixed(2)}</Text></View>
                {receiptSale?.discount > 0 && (
                  <View style={styles.receiptRow}><Text style={styles.receiptLabel}>{t('Discount')}</Text><Text style={[styles.receiptValue, { color: '#16A34A' }]}>-₹{receiptSale.discount.toFixed(2)}</Text></View>
                )}
                <View style={[styles.receiptRow, styles.receiptGrandTotalRow]}>
                  <Text style={styles.receiptGrandTotalLabel}>{t('Grand Total')}</Text>
